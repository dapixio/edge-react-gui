...
import { confirmRequest, getFioWalletByFioAddress } from '../../modules/FioRequest/action'
...
const mapStateToProps = (state: State) => {
  ...
  const fiatSymbol = ''
  let fromCurrencyCode, fromButtonText: string, fromFiatToCrypto: number
  if (fromWallet) {
    ...
    fromCurrencyCode = ''
    fromButtonText = s.strings.fio_src_wallet
    fromFiatToCrypto = 1
  }

  const exchangeRates = state.exchangeRates
  const fromCurrencyIcon = ''
...
const mapDispatchToProps = (dispatch: Dispatch): DispatchProps => ({
  ...
  getFioWalletByFioAddress: (payerPublicAddress: string) => {
    dispatch(getFioWalletByFioAddress(payerPublicAddress))
  }
})
