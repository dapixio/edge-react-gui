...
export class Request extends Component<Props, State> {
  ...
  fioAddressModal = async () => {
    ...
    if (this.fioWallets.length === 0) {
      showError(`${s.strings.title_register_fio_address}. ${s.strings.fio_request_by_fioaddress_error_no_address}`)
      return
    }
    ...
    if (this.amounts) {
      ...
      if (native <= 0) {
        showError(`${s.strings.fio_request_by_fioaddress_error_invalid_amount_header}. ${s.strings.fio_request_by_fioaddress_error_invalid_amount}`)
        return
      }
    } else {
      showError(`${s.strings.fio_request_by_fioaddress_error_invalid_amount_header}. ${s.strings.fio_request_by_fioaddress_error_invalid_amount}`)
    ...
    const fioAddressModal = createFioAddressModal({ engine, isConnected: this.props.isConnected })
