...
export type DispatchProps = {
  ...
  fioRejectRequest: (fioRequestId: string, payerFioAddress: string, payeeFioAddress: string, cb: Function) => void,
...
export class FioRequestList extends Component<Props, State> {
  ...
  rejectFioRequest = (rowMap: any[], rowKey: any, requestId: string, payerFioAddress: string, payeeFioAddress: string) => {
    if (!this.props.isConnected) {
      showError(s.strings.fio_network_alert_text)
      return
    }
    this.setState({ rejectLoading: true })
    this.props.fioRejectRequest(`${requestId}`, payerFioAddress, payeeFioAddress, e => {
      if (!e) {
  ...
  rejectRow = (rowMap: any[], rowKey: any, requestId: string, payerFioAddress: string, payeeFioAddress: string) => {
    Alert.alert(
      ...
      [
        ...
        { text: s.strings.fio_reject_request_yes, onPress: () => this.rejectFioRequest(rowMap, rowKey, requestId, payerFioAddress, payeeFioAddress) }
      ],
      { cancelable: false }
    )
  }
                  ...
                  <View style={requestListStyles.rowBack}>
                    <TouchableOpacity
                      ...
                      onPress={_ =>
                        this.rejectRow(
                          rowMap,
                          transaction.item.fio_request_id.toString(),
                          transaction.item.fio_request_id,
                          transaction.item.payer_fio_address,
                          transaction.item.payee_fio_address
                        )
                      }
                    >
