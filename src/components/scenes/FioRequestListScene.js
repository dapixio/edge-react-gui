...
import { Alert, Animated, FlatList, Image, TouchableHighlight, TouchableOpacity, View } from 'react-native'
...
import type { CheckConnectivityProps, GoToSceneProps, GuiWallet } from '../../types/types'
...
type Props = StateProps & DispatchProps & CheckConnectivityProps & GoToSceneProps
...
export class FioRequestList extends Component<Props, State> {
  ...
  rejectRow = (rowMap: any[], rowKey: any, requestId: string, payerFioAddress: string) => {
    Alert.alert(
      ...
      [
        ...
        { text: s.strings.fio_reject_request_message, onPress: () => this.rejectFioRequest(rowMap, rowKey, requestId, payerFioAddress) }
      ],
      { cancelable: false }
    )
  }
  ...
  headerTitle = (headerDate: Date) => {
    return getMonthName(headerDate.getMonth(), true) + ' ' + headerDate.getDate() + ', ' + headerDate.getFullYear()
  }
  ...
  headerRow = (headerDate: Date) => {
    return (
      <View style={requestListStyles.rowContainer}>
        <T style={requestListStyles.rowTitle}>{getMonthName(headerDate.getMonth(), true) + ' ' + headerDate.getDate() + ', ' + headerDate.getFullYear()}</T>
      </View>
    )
  }
  ...
  selectRequest = (tx: Object) => {
    if (!this.props.checkConnectivity()) return
    const { wallets, setFioPendingRequestSelected, goToScene } = this.props
    for (const walletKey: string of Object.keys(this.props.wallets)) {
      if (wallets[walletKey].currencyCode.toLowerCase() === tx.content.token_code.toLowerCase()) {
        ...
        goToScene(Constants.FIO_PENDING_REQUEST_DETAILS)
        return
      }
    }
    Alert.alert(s.strings.err_token_not_in_wallet_title, s.strings.err_token_not_in_wallet_msg)
  }

  selectSentRequest = (tx: Object) => {
    if (!this.props.checkConnectivity()) return
    this.props.setFioSentRequestSelected(tx)
    this.props.goToScene(Constants.FIO_SENT_REQUEST_DETAILS)
  }
