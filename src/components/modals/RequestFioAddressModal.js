...
export class FioAddressModal extends Component<AddressModalProps, AddressModalState> {
  ...
  _setClipboard = async () => {
    try {
      ...
      const isFioAddress = await this.props.engine.otherMethods.validateFioAddress(address)
  ...
  validateAddress = async (): Promise<boolean> => {
    ...
    if (address && address.length >= 3 && address.length <= 100 && (await this.props.engine.otherMethods.validateFioAddress(address))) {
      this.setState({
        addressError: '',
        addressLabel: s.strings.fio_address_label,
        addressFocus: false,
...
export type FioAddressModalOpts = { engine: any, isConnected: boolean }
